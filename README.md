# 培智·双脉教学引擎（特教 IEP 备课 Skill）V2.4.0
...
内置**红区隐私护栏**、**双 Gate 低打扰确认协议**、**跨课时行为干预递进（PBS）**、**无障碍排版基线**与 **4×3 知识类型策略路由**；机器可读产物遵循 `docs/output-schema.json` 结构化契约。
...
| 成品一键生成 | `python docs/md_to_docx.py`：md → docx 的**唯一执行通道**（零第三方依赖、字节幂等、红区命中拒绝落盘），A4/页边距/字号行距/中西文字体/表头跨页重复/列宽比/封面独立节/页脚页码全部由代码强制；`--pdf` 自动尝试 Word COM → docx2pdf → LibreOffice，皆无则给出降级指引；`--check` 校验成品是否被改动或失同步 |
...
| 结构化输出契约 | 机器可读产物合 `docs/output-schema.json`（JSON Schema draft 2020-12），支持**多锚点** anchors，含 timeline 步别枚举与时间恒等硬约束，可直接对接教务系统 |
| Single Source 派生 | 结构化 JSON **只可由 md 源稿经 `docs/md_to_json.py` 派生**，禁止手工样例；回归自动比对"落盘样例≡派生结果"防漂移 |
...
│   ├── md_to_json_report.txt      # 最近一次派生结果
│   ├── md_to_docx.py              # md 源稿 → Word 成品生成器（唯一执行通道，零依赖/字节幂等）
│   ├── md_to_docx_report.txt      # 最近一次生成或一致性校验结果
│   ├── regression_check.py        # 回归校验脚本（教案/契约/引擎自身/派生一致性/成品/仓库卫生）
│   └── regression_report.txt      # 最近一次回归校验结果（含通过率）
├── examples/
│   ├── 认识5_教学设计方案_2课时.md             # 多课时基准（生活数学）
│   ├── 认识5_教学设计方案_2课时.docx           # 由 md 生成的 Word 成品（回归基准，须入库）
│   ├── 好吃的水果_教学设计方案_3课时.md        # 多课时基准（生活语文）
│   ├── 好吃的水果_教学设计方案_3课时.docx      # 由 md 生成的 Word 成品（回归基准，须入库）
...
- 版本：2.4.0（2026-09-24 第四轮三视角回测修订：格式基线代码化＋感官调节前置＋IEP 累计追踪＋Gate 话术模板＋上下文预算与幂等策略；回归校验全项通过，以 `docs/regression_report.txt` 末尾通过率为准），明细见 [CHANGELOG.md](CHANGELOG.md)。
- 回归校验：`python docs/regression_check.py`，输出 `docs/regression_report.txt`，**发布前须 100% 通过**（项数随版本增长，以报告为准，不硬编码）。
- 结构化 JSON 派生：`python docs/md_to_json.py`，输出 `docs/md_to_json_report.txt`；**改教案请先改 md 再派生**，禁止手工编辑 JSON。
- Word 成品生成：`python docs/md_to_docx.py`（加 `--pdf` 导出 PDF，加 `examples --check` 校验成品一致性）。顺序恒为：改 md → 派生 JSON → 生成 docx → 跑回归。
- 主线"六环节＋双 Gate＋四视图＋双格式"各版本保持不变；2.0.0 起 SKILL.md 仅保留运行态引擎（治理内容外置到 LICENSE/CONTRIBUTING/docs）。
